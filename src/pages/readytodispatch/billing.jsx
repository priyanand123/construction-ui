...
const Billing = () => {
  ...
  const createData = async (formData) => {
    try {
      ...
      const IsGSTInclude =
        formData?.isGSTInclude == true || formData?.isGSTInclude == 1 ? 1 : 0;
      const dataBody = {
        id: formData?.id || 0,
  ...
  const updateData = async (formData) => {
    try {
      ...
      if (formData?.isGSTInclude == true || formData?.isGSTInclude == 1) {
  ...
  const columns = React.useMemo(
    () => [
      ...
      {
        Header: "Bill Type",
        accessor: "isGSTInclude",
        Cell: (props) => (props.value ? "GST" : "No GST"),
        Filter: TextSearchFilter,
