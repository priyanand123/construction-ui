...
export const fetchstockListReq = async () => {
  try {
    const res = await publicAxios.get(`${ApiKey.getStock}`);
...
export const fetchUserByIdReq = async (userId) => {
  try {
    const res = await authAxios.get(`${ApiKey.getStock}/${userId}`);
...
export const updatestockListReq = async (data) => {
  try {
    debugger;
    const res = await publicAxios.put(`${ApiKey.updateStock}`, data);
