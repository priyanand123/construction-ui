...
export const fetchmaterialHistoryListReq = async () => {
  try {
    const res = await publicAxios.get(`${ApiKey.getMaterialHistory}`);
...
export const fetchUserByIdReq = async (userId) => {
  try {
    const res = await authAxios.get(`${ApiKey.getMaterialHistory}/${userId}`);
